...
//  IMPORTANT: The range must be sorted. This will not be checked.
template<typename T>
...
void vec_mark_outlier_mehod2(std::vector<T> &vec, std::vector<bool> &outlier)
{
  ...
  std::cout << "Qi: "  << Q1 << " " << Q2 << " " << Q3 << '\n';
  std::cout << "Bounds: "  << O_lower << " " << O_upper << '\n';
}
...
// Side effect: vec gets sorted. More efficient than copying the vector.
template<typename T>
size_t vec_median_quartile_outlier_bounds_sort_method3(std::vector<T> &vec, double &Q1, double &Q2, double &Q3, double &O_lower, double &O_upper, float IQR_factor=1.5, float min_IQR = 0)
{
  ...
  O_lower = Q1 - IQR_factor * IQR;
  O_upper = Q3 + IQR_factor * IQR;
...
// Same vec_median_quartile_outlier_bounds_sort but without sorting the vector. Slighly slower, since the vector has to be copied.
template<typename T>
size_t vec_median_quartile_outlier_bounds_method3(std::vector<T> vec, double &Q1, double &Q2, double &Q3, double &O_lower, double &O_upper, float IQR_factor=1.5, float min_IQR = 0)
{
  int    res = vec_median_quartile_sort_method3(vec, Q1, Q2, Q3);
  double IQR = Q3 - Q1;
