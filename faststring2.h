...
/*   } */
  ...
  {
    res.clear();
    size_t pos1 = find(delim1,0);
    if (pos1 == npos)
      return false;
    size_t pos2 = find(delim2,pos1+delim1.size());
    if (pos2 == npos)
      return false;

    std::cout << "pos1: " << pos1 << " pos2: " << pos2 << std::endl;

    unsigned s = pos1+delim1.size();
    unsigned e = pos2;
...
//    char *e = (char *) end();
  ...
  void replace_chars(faststring &replace)
  {
    if (replace.length()%2 != 0)
    {
      return;
    }
    for (unsigned i=0; i<replace.length(); i+=2)
    {
